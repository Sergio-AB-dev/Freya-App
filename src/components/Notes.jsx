...
const Notes = () => {
  ...
  const handleSave = async () => {
    try {
      if (editingId) {
        const noteDoc = doc(db, 'notes', editingId);
        await updateDoc(noteDoc, modalData);
        setNotes(notes.map(note =>
          note.id === editingId ? { ...note, ...modalData } : note
        ));
      } else {
        const docRef = await addDoc(collection(db, 'notes'), modalData);
        setNotes([...notes, { ...modalData, id: docRef.id }]);
      }
      closeModal();
    } catch (error) {
            ...
            <div className="apuntes__continer--Creacion__asignatura">
              ...
              <div className="apuntes__continer--Creacion__botones">
                ...
                <button className="apuntes__guardar" onClick={handleSave}>
                  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 507.506 507.506" width="512" height="512">
